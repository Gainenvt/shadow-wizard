       ...
       if (transform.position.y < -4f)
{
    ...
    else
    {
        GameManager.Instance.CircleMissed();
            ...
            // Play a sound right before the orb disappears at the bottom
            if (orbAudioSource != null && orbLockSound != null)
            {
                // We use PlayClipAtPoint so the sound stays alive even after the orb is gone
                AudioSource.PlayClipAtPoint(orbLockSound, transform.position);
            }
            Destroy(gameObject);
}
    }
    ...
    void CheckMatch()
    {
        ...
        if (group.Count >= 3)
        {// Play Match Sound
            if (orbAudioSource != null && orbMatchSound != null)
            {
                orbAudioSource.PlayOneShot(orbMatchSound);
            }
    ...
    {// Play Lock Sound
        if (orbAudioSource != null && orbLockSound != null)
        {
            orbAudioSource.PlayOneShot(orbLockSound);
        }
        if (isFrozen)
            return;
